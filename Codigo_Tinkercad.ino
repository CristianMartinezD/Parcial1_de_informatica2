...
void loop()
{
  if(condi == 0){
    ...
    TotalF = Totalfiguras();
    int figuras[TotalF][64] = {};
    
    for(int i = 0; i < TotalF; i++){
      Serial.println("INGRESA TODOS LRDS DE LA FIGURA #"+String(i+1));
      vector = construirArray();
      	
      for(int j = 0; j < 64; j++){
        figuras[i][j] = vector[j];
      }
    }
...
int* construirArray(){
  ...
  while(ban != 0){
   cont += 1;
   Serial.println(" Ingresa el led #"+String(cont)+" o ingresa 0 para fializar.");
...
int LEDS(){
  ...
  while(ban == 0){
    if(Serial.available()){
      Serial.println("Por favor espera un momento......");
       LED = Serial.parseInt();
      ban = 1;
    }
  }
